...
#COM PCA
rmse_npcs = []  #Para guardar o RMSE de cada N de pcs
r2_npcs = []    #Para guardar o R2 de cada N de pcs
...
#=====================================================
n_pcs_list = range(1,database.shape[1])           #tamanho de n_pcs
...
for n_pcs in  n_pcs_list:
    ...
    for train_index, test_index in k_folds.split(X):        # K-fold cria os indices train_index e test_index em formato array
        ...
        #==================================================================================================================================
    rmse_npcs.append(np.mean(rmse_folds))
    r2_npcs.append(np.mean(r2_folds))
# --- Resultados finais ---
print("RMSE médio de cada nPC:", rmse_npcs)
print("R² médio de cada nPC:", r2_npcs)

plt.figure(figsize=(8,5))
plt.plot(list(n_pcs_list), rmse_npcs, marker='o', label='RMSE médio')
plt.xlabel('Número de Componentes Principais (nPC)')
plt.ylabel('RMSE médio')
plt.title('Validação do número de PCs usando K-Fold')
plt.xticks(range(1,12,1))
plt.grid(True)
plt.legend()
plt.show()

# Opcional: Plot R² no mesmo gráfico ou separado
plt.figure(figsize=(8,5))
plt.plot(list(n_pcs_list), r2_npcs, marker='o', color='orange', label='R² médio')
plt.xlabel('Número de Componentes Principais (nPC)')
plt.ylabel('R² médio')
plt.title('Validação do número de PCs usando K-Fold')
plt.xticks(range(1,12,1))
...
n_pcs_otimo = n_pcs_list[np.argmin(rmse_npcs)]
print(f"Número de PC's ótimo = {n_pcs_otimo}")