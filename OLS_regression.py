# Primeiro análise e pré processamento e então aplicação do modelo OLS implemnetado a mão
...
# ✅ (Z-SCORE) ================================================================================================================
# Normalização média 0 e desvio padrao 1 para realizar o calculo equilibrado da matriz de covariância.
# Estabilidade numerica ao colocar todo mundo na mesma escala nada explode e ainda ajuda caso va usar ridge ou lasso pra 
# penalizar todo mundo igual
...
# IMPORTANTE USAR OS VALORES DO MODELO DE TREINO DE STD E MEAN PARA O CONJUNTO DE TESTE QUANDO FOR FAZER O Z-SCORE DENOVO.
#===============================================================================================================================
...
x_train_bias = np.column_stack([np.ones(x_train_ajusted_norm.shape[0]), x_train_ajusted_norm])  #adiciona uma coluna de 1's para representar o b0

# aplicação da fórmula do OLS [(Xt*X)^-1 * Xt * Y]
betas = np.linalg.inv(  x_train_bias.T     @       x_train_bias )     @       (x_train_bias.T    @   y_train)
...
y_predict_train =  x_train_bias @ betas
...
# Bias (nada muda)
x_test_ajusted_norm = np.column_stack([np.ones(x_test_ajusted_norm.shape[0]), x_test_ajusted_norm])

# Aplicação do modelo para validação e (MSE) de validação no teste
y_predict_test = x_test_ajusted_norm @ betas
